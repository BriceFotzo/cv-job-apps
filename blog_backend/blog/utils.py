...
def matching_pipeline(job_link,user,pwd,path,content,upload):
    if(upload):
        resume_content = docx2txt.process(path)
    else:
    ...
    for head,job in zip(heads,links):
        ...
        data=match_resume_and_job(job_content,resume_content,head,cpt)
        data['job_link']=job
    ...
    print(job_list)
    return job_list
def get_data_and_find_jobs(username,password,job_query_online,resume_path,resume_content,upload):
    keyword_job=quote(job_query_online)
    jobs="https://www.linkedin.com/jobs/search/?keywords="+keyword_job+"&location=France"
    matching_pipeline(job_link=jobs,user=username,pwd=password,path=resume_path,content=resume_content,upload=upload)
